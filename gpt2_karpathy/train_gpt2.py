...
train_dataloader = DataLoaderLite(
    B=B, T=T, process_rank=dpp_rank, num_processes=dpp_world_size, split="train"
)
val_dataloader = DataLoaderLite(
    B=B, T=T, process_rank=dpp_rank, num_processes=dpp_world_size, split="val"
)

assert (
    total_batch_size % (B * T * dpp_world_size) == 0
), "total batch size must be divisible by B * T * dpp_world_size"
grad_accum_steps = total_batch_size // (B * T * dpp_world_size)
if config.master_process:
    print(
        f"total batch size: {total_batch_size}, grad accum steps: {grad_accum_steps}, 1 epoch steps is {len(train_dataloader.tokens) / (B * T * grad_accum_steps * dpp_world_size)}"
    )
...
optimizer = raw_model.configure_optimizers(
    weight_decay=0.1, learning_rate=3e-4, device_type=device_type
)
...
for i in range(max_steps):
    ...
    tokens_processed = (
        train_dataloader.B * train_dataloader.T * grad_accum_steps * dpp_world_size
    )
