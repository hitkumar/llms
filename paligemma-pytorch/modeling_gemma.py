...
class KVCache:
    ...
    def update(
        ...
        if len(self.key_cache) <= layer_idx:
            # We never added anything to this layer of the cache before.
...
def repeat_kv(hidden_states: torch.tensor, n_rep: int) -> torch.tensor:
    ...
    return (
        hidden_states[:, :, None, :, :]
        .expand(bs, num_kv_attention_heads, n_rep, seq_len, head_dim)
        .reshape(bs, -1, seq_len, head_dim)
    )
    ...
    def forward(
        ...
        **kwargs,
        ...
        value_states = value_states.view(
            bs, seq_len, self.num_key_value_heads, self.head_dim
        ).transpose(1, 2)
        ...
        if kv_cache is not None:
            key_states, value_states = kv_cache.update(
                key_states, value_states, self.layer_dx
            )
        ...
        attn_output = attn_output.transpose(1, 2).contiguous().view(bs, seq_len, -1)
...
class PaliGemmaForConditionalGeneration(nn.Module):
    ...
    def forward(self, input_ids, pixel_values, attention_mask, kv_cache):
        ...
        input_embeds, causal_mask, position_ids = (
